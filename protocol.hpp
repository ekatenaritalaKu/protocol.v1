...
namespace eosio {
    ...
    static const account_name _verificator = N(operatordcm);


    static const eosio::symbol_name _SYM = S(4, EOS);
    static const uint64_t _MAX_SUPPLY = 100000000000000;

    // @abi table account
    struct account{
        //ToDo Add first pool cost, total cycles, total money recycle
        ...
        EOSLIB_SERIALIZE( account, (username)(purpose)(registered_at)(verified)(activated))
    };
    ...
    struct balance{
        ...
        bool is_empty()const { return !( purchase_amount.amount | sold_amount.amount); }
    ...
    struct pool{
        ...
        bool is_empty()const { return !( total_in_box.amount | lept_cost.amount | next_pool.amount | total_win_withdraw.amount |total_loss_withdraw.amount ); }
        uint64_t primary_key() const {return id;}
    ...
    struct rate {
        ...
        bool is_empty()const { return !( client_income.amount | delta.amount | pool_cost.amount | total_in_box.amount |payment_to_wins.amount  | payment_to_loss.amount | system_income.amount | live_balance_for_sale.amount  ); }
