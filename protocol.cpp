...
void start_new_cycle ( account_name username, account_name host){
    ...
    void next_pool( account_name username, account_name host){
        ...
        if (dprop -> current_pool_num > 2) {
            ...
            reserved_lepts = (prev_prev_pool -> released_lepts * rate -> sell_rate / rate -> buy_rate  \
            + prev_prev_pool -> total_reserved_lepts * rate -> sell_rate / rate -> buy_rate) / LEPTS_PRECISION * LEPTS_PRECISION;
    ...
    void verify_action(const verify &op){
        require_auth(_verificator);
        
        account_index accounts(_self, _self);

        auto acc = accounts.find(op.host);
        
        accounts.modify(acc, _verificator, [&](auto &a){
            a.verified = true;
        });
    ...
    void setfee_action(const setfee &op){
        ...
        eosio_assert(op.createhost.symbol == _SYM, "Rejected");
        eosio_assert(op.systemfee < 10000, "Rejected");
    ...
    void setparams_action(const setparams &op){
        ...
        eosio_assert((pool_timeout >= 60) && (pool_timeout < 7884000),"Pool Timeout must be greater or equal then 60 sec and less then 7884000");
    ...
    void deposit ( account_name username, account_name host, eosio::asset amount){
        ...
        eosio_assert ( amount.symbol == _SYM, "Rejected. Only Core token is accepted by DACom Protocol for now.");
    ...
    void apply( uint64_t /*receiver*/, uint64_t code, uint64_t action ) {
      ...
      if (code == _self) {
         ...
         if (action == N(verify)){
            verify_action(eosio::unpack_action_data<verify>());
         }
     };
